...
class ScreenBase:
    def __init__(self, app):
        self.app = app
        self.game = None  # Screens that need a Game will set this
...
class App:
    ...
    def change_screen(self, new_screen):
        """Single place for switching screens."""
        self.screen = new_screen

        # Keep legacy code working: many entities/functions reference global `game`
        global game
        if hasattr(new_screen, "game") and new_screen.game is not None:
            game = new_screen.game

    def _build_input_state(self) -> InputState:
...
class MenuScreen(ScreenBase):
    def __init__(self, app):
        super().__init__(app)
        # Game created INSIDE screen transition / screen init (not global update)
        self.game = Game()  # no player on menu
...
class PlayScreen(ScreenBase):
    def __init__(self, app):
        super().__init__(app)
        # Create a new game + player when entering play
        self.game = Game(Player())
        self.paused = False


    def update(self, input_state):
        # Toggle pause (only in PlayScreen)
        if input_state.pause_pressed:
            self.paused = not self.paused
            return  # important: don't update game on the toggle frame
    ...
    def draw(self):
        ...
        if self.paused:
            # simple overlay text in the middle
            screen.draw.text(
                "PAUSED",
...
class GameOverScreen(ScreenBase):
    def __init__(self, app, existing_game):
        super().__init__(app)
        # Reuse the last game so status displays properly
